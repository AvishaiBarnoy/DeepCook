...
class TestFilterKosher:
    ...
    def meals_with_kosher(self):
        """Create a meal database with various kosher types."""
        data = {
            'Name': ['Pasta Salad', 'Cheeseburger', 'Steak', 'Chicken Soup', 'Bacon', 'Hummus'],
            'Kosher': ['parve', 'milchik', 'fleisch', 'fleisch', 'nonkosher', 'parve'],
            'Rank': [7, 8, 9, 8, 6, 7],
        }
        return pd.DataFrame(data)
    
    def test_filter_nonkosher_shows_all(self, meals_with_kosher):
        """Test that nonkosher filter shows all meals."""
        result = filter_kosher(meals_with_kosher, KosherType.nonkosher)
        assert len(result) == 6
        assert set(result['Name']) == {'Pasta Salad', 'Cheeseburger', 'Steak', 'Chicken Soup', 'Bacon', 'Hummus'}
    
    def test_filter_parve_excludes_meat_dairy(self, meals_with_kosher):
        """Test that parve filter only shows parve and nonkosher."""
        result = filter_kosher(meals_with_kosher, KosherType.parve)
        assert len(result) == 3
    ...
    def test_filter_milchik_excludes_meat(self, meals_with_kosher):
        """Test that milchik filter excludes fleisch."""
        result = filter_kosher(meals_with_kosher, KosherType.milchik)
        assert len(result) == 4
    ...
    def test_filter_fleisch_excludes_dairy(self, meals_with_kosher):
        """Test that fleisch filter excludes milchik."""
        result = filter_kosher(meals_with_kosher, KosherType.fleisch)
        assert len(result) == 5
